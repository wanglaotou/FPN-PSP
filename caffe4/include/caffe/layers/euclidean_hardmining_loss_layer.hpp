#ifndef CAFFE_EUCLIDEAN_HARDMINING_LOSS_LAYER_HPP_
#define CAFFE_EUCLIDEAN_HARDMINING_LOSS_LAYER_HPP_
...
class EuclideanHardminingLossLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanHardminingLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanHardminingLoss"; }
 ...
 protected:
  /// @copydoc EuclideanHardminingLossLayer
  ...
  float hard_sample_ratio_;
  std::vector<Dtype> hard_sample_loss_vec_;

  static bool SortByLoss(const Dtype& v1  ,const Dtype& v2);
  void PushLoss(std::vector<Dtype> &loss_vec, const Dtype* diff_data, int count);
  void PrintLoss(std::vector<Dtype> &loss_vec);
};

}  // namespace caffe

#endif  // CAFFE_EUCLIDEAN_HARDMINING_LOSS_LAYER_HPP_
