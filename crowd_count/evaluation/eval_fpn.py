...
def fileMap(conf, path_data, path_annot, path_fn_save, path_fd_dmap, mre_thre, is_src=False, if_do_mean_scale=False, mean_value=[127.5, 127.5, 127.5], 
    ...
    with open(path_annot, 'r') as fm:
        ...
        for m_line in map_list:
            ...
            restCols = orgCols % 64
            restRows = orgRows % 64
            extCols = (64 - restCols)%64
            extRows = (64 - restRows)%64
            wrap = img_process(img, model.net_name, is_test_half)
            im = cv2.copyMakeBorder(wrap,0,extRows,0,extCols,cv2.BORDER_CONSTANT)
