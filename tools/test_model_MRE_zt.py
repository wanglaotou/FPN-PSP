...
#caffe_root = '/home/zhangting/zhangting/code_exp/crowd_count/exp/MCNN3.1/caffe/'
caffe_root = '../caffe/'
...
def fileMap(filepath,netFile,caffeModel,saveFile,saveMapFile,isSaveHeatMap=False):
    ...
    for m in mapList:
        ...
        if(os.path.isfile(jpgDir)):
            ...
            ## 2> set net img size
            # print 'net shape before:',net.blobs['data'].data.shape
            net.blobs['data'].reshape(1,im.shape[2],h,w)
            ...
            if isSaveHeatMap==True:
                ...
                if not os.path.exists(newImgPath):
                    os.mkdir(newImgPath)
                matplotlib.image.imsave(os.path.join(newImgPath,imgNameIndex[0]+'.jpg'), feat1)
...
def find_model_iter(dir, iter):
    path_ret = None
    for root, dirs, files in os.walk(dir):
        for file in files:
            if (file.find(iter)!=-1) and (file.find('.caffemodel')!=-1):
                path_ret = os.path.join(root, file)
                break;
    return path_ret

if __name__=='__main__':
    workPath  = sys.argv[1]
    iter_test = sys.argv[2]
    dataPath = '/ssd/zhangting/crowdcount/data'

    netFile = os.path.join(workPath,'deploy.prototxt')
    path_mdl = find_model_iter(workPath, iter_test);
    if path_mdl is None:
        print 'can not find model of iter: ' + iter_test
        sys.exit()
    caffeModel = path_mdl
    caffe.set_device(3)
    caffe.set_mode_gpu()       
    ...
    if not os.path.exists(savePath):
        os.mkdir(savePath)
    saveFile = os.path.join(savePath,'test_%dw.txt'%(int(iter_test)/10000))
    saveMapFile = os.path.join(workPath,'showMap')
    
    fileMap(dataPath,netFile,caffeModel,saveFile,saveMapFile,True)
